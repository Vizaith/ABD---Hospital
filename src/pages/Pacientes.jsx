...
export const Pacientes = () => {
  ...
  const registrar = async () => {
    ...
    if (editingId) {
      await supabase.from('pacientes').update(form).eq('id_paciente', editingId);
    } else {
  ...
  const editar = (item) => {
    ...
    setEditingId(item.id_paciente); setView('form');
  };
  ...
  return (
    <div className="form-card">
      ...
      <button className="btn-submit" disabled={!isFormValid} onClick={registrar} style={{ opacity: isFormValid ? 1 : 0.5 }}>
